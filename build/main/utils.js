...
const fieldIsObjectOrListOfObject = (field) => field.type.kind === 'OBJECT' ||
    (field.type.ofType &&
    ...
    return inputFields.reduce((current, next) => {
        ...
        if (fieldType) {
            ...
            if (valueMapperForType) {
                return Object.assign(Object.assign({}, current), { [key]: valueMapperForType(input[key]) });
            }
        }
...
exports.createQueryFromType = (type, typeMap, allowedTypes) => {
    return typeMap[pluralize_1.singular(type)].fields.reduce((current, field) => {
        ...
        if (fieldIsObjectOrListOfObject(field)) {
            const thisType = field.type.ofType && // We also handle cases where we have e.g. [TYPE!] (List of type)
            ...
            if (typeName && allowedTypes.indexOf(typeName) !== -1) {
                return `
        ${current} ${field.name} {${exports.createQueryFromType(typeName, typeMap, allowedTypes)} }
        `;
            }
...
exports.createGetManyQuery = (type, manyLowerResourceName, resourceTypename, typeMap, queryMap, allowedTypes, idType) => {
...
exports.createGetListQuery = (type, manyLowerResourceName, resourceTypename, typeMap, queryMap, allowedTypes) => {
    if (!exports.queryHasFilter(manyLowerResourceName, queryMap)) {
