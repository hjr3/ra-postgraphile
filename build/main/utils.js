...
const shouldQueryField = (fieldName, typeConfig) => {
...
exports.createQueryFromType = (type, typeMap, typeConfiguration, primaryKey) => {
    return typeMap[type].fields.reduce((current, field) => {
        ...
        if (fieldIsObjectOrListOfObject(field)) {
            const thisType = ((_a = field.type.ofType) === null || _a === void 0 ? void 0 : _a.name) ? field.type.ofType
                : // We also handle cases where we have e.g. [TYPE!] (List of type)
                 (_b = field.type.ofType) === null || _b === void 0 ? void 0 : _b.ofType;
            const typeName = (thisType && thisType.name) || field.type.name;
            const shouldExpand = typeName && typeConfiguration[typeName] && typeConfiguration[typeName].expand;
            if (typeName && shouldExpand) {
                return `
        ${current} ${field.name} {${exports.createQueryFromType(typeName, typeMap, typeConfiguration, primaryKey)} }
        `;
            }
...
exports.createGetManyQuery = (type, manyLowerResourceName, resourceTypename, typeMap, queryMap, typeConfiguration, primaryKey) => {
    if (!exports.queryHasArgument(manyLowerResourceName, ARGUMENT_FILTER, queryMap)) {
      ...
      nodes {
        ${exports.createQueryFromType(resourceTypename, typeMap, typeConfiguration, primaryKey)}
      }
    }
    }`;
    }
...
exports.createGetListQuery = (type, manyLowerResourceName, resourceTypename, pluralizedResourceTypeName, typeMap, queryMap, typeConfiguration, primaryKey) => {
