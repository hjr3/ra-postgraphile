...
let typeMap;
let queryMap;
exports.mapType = (idType, value) => ['uuid', 'string'].includes(idType.name.toLowerCase())
    ? value
    : parseInt(value, 10);
exports.buildQuery = (introspectionResults, factory) => (raFetchType, resName, params) => {
    ...
    const manyLowerResourceName = pluralize_1.default(utils_1.lowercase(resourceTypename));
    ...
    switch (raFetchType) {
        case types_1.VERB_GET_ONE:
            return {
                query: graphql_tag_1.default `query ${singleLowerResourceName}($id: ${idType.name}!) {
            ${singleLowerResourceName}(id: $id) {
            ${utils_1.createQueryFromType(resourceTypename, typeMap, allowedComplexTypes)}
        }
        }`,
                variables: {
        ...
        case types_1.VERB_GET_MANY:
            return {
                query: utils_1.createGetManyQuery(type, manyLowerResourceName, resourceTypename, typeMap, queryMap, allowedComplexTypes, idType.name),
                variables: {
                    ids: params.ids
        ...
        case types_1.VERB_GET_MANY_REFERENCE:
            return getManyReference_1.getManyReference(params, type, manyLowerResourceName, resourceTypename, typeMap, queryMap, allowedComplexTypes);
        case types_1.VERB_GET_LIST: {
            ...
            return {
                query: utils_1.createGetListQuery(type, manyLowerResourceName, resourceTypename, typeMap, queryMap, allowedComplexTypes),
                variables: {
        ...
        case types_1.VERB_CREATE: {
            const variables = {
                input: {
                    [singleLowerResourceName]: utils_1.mapInputToVariables(params.data, typeMap[`${resourceTypename}Input`], type, options.queryValueToInputValueMap)
                }
            };
            return {
                variables,
        ...
        case types_1.VERB_DELETE_MANY: {
            ...
            return {
                variables: deletions.reduce((next, input) => (Object.assign({ [`arg${input.id}`]: input }, next)), {}),
                query: graphql_tag_1.default `
        ...
        `,
                parseResponse: (response) => ({
                    data: params.ids.map((id) => exports.mapType(idType, response.data[`k${id}`].clientMutationId))
                })
            };
        }
        case types_1.VERB_UPDATE: {
            const updateVariables = {
                input: {
                    id: exports.mapType(idType, params.id),
                    patch: utils_1.mapInputToVariables(params.data, typeMap[`${resourceTypename}Patch`], type, options.queryValueToInputValueMap)
                }
            };
            return {
        ...
        case types_1.VERB_UPDATE_MANY: {
            ...
            const inputs = ids.map(id => ({
                id: exports.mapType(idType, id),
                clientMutationId: id.toString(),
                patch: utils_1.mapInputToVariables(data, typeMap[`${resourceTypename}Patch`], type, options.queryValueToInputValueMap)
            }));
            return {
                variables: inputs.reduce((next, input) => (Object.assign({ [`arg${input.id}`]: input }, next)), {}),
        ...
        ${ids
          ...
          ${inputs.map(input => {
            ...
            `;
                ...
                parseResponse: (response) => ({
                    data: ids.map(id => exports.mapType(idType, response.data[`update${id}`].clientMutationId))
                })
            };
        }
        default:
