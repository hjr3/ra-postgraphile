...
export var mapInputToVariables = function(input, inputType, type, typeMapper) {
  ...
  return inputFields.reduce(function(current, next) {
    ...
    var fieldType = type.fields.find(function(field) {
      return fieldIsObjectOrListOfObject(field) && field.name === key
    })
    if (fieldType) {
      ...
      if (valueMapperForType) {
        return __assign(
          __assign({}, current),
          ((_a = {}), (_a[key] = valueMapperForType(input[key])), _a)
        )
      }
    }
    return __assign(
      __assign({}, current),
      ((_b = {}), (_b[key] = input[key]), _b)
    )
  }, {})
}
export var queryHasFilter = function(type, queryMap) {
  ...
  return Boolean(
    queryMap[type].args.find(function(f) {
    ...
    if (fieldIsObjectOrListOfObject(field)) {
      ...
      if (typeName && allowedTypes.indexOf(typeName) !== -1) {
        return (
          ...
          field.name +
          ' {' +
          createQueryFromType(typeName, typeMap, allowedTypes) +
          ' }\n        '
        )
      }
...
export var createGetManyQuery = function(
  type,
...
export var createGetListQuery = function(
  type,
  manyLowerResourceName,
  resourceTypename,
  typeMap,
