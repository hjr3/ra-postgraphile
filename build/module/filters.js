...
var filterMappings = {
    bigint: {
        equalTo: typeof BigInt !== 'undefined' ? BigInt : function (value) { return value; }
    },
    bigintarray: {
        in: function (value) {
    ...
    datetime: {
        isNull: function (value) { return Boolean(value); }
    },
    int: {
        equalTo: Number
    },
    intarray: {
        in: function (value) { return value.map(Number); }
    },
    intlistarray: {
        contains: function (value) { return value.map(Number); }
    },
    string: {
        equalTo: String,
        includes: function (value) { return "" + value; },
        includesInsensitive: function (value) { return "" + value; },
        like: function (value) { return "" + value; },
        likeInsensitive: function (value) { return "" + value; }
    },
    stringlistarray: {
        contains: function (value) { return value.map(String); },
        equalTo: function (value) { return value.map(String); }
    },
    uuid: {
        equalTo: function (value) { return value; }
    },
    uuidarray: {
        in: function (value) { return value; }
    },
    fulltext: {
        matches: function (value) { return value + ":*"; }
    }
};
/**
...
export var mapFilterType = function (type, value, key) {
    ...
    var _d = filterMaps[0], operator = _d[0], v = _d[1];
    ...
    var filters = Object.keys(fields).reduce(function (next, key) {
        ...
        if (maybeType) {
            ...
            if (maybeType.type.kind === 'LIST') {
                thisType = __assign(__assign({}, thisType), { name: maybeType.type.ofType.name + "List" });
            }
