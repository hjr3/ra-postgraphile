...
import { capitalize, createGetListQuery, createGetManyQuery, createQueryFromType, createSortingKey, createTypeMap, lowercase, mapInputToVariables } from './utils';
import { NATURAL_SORTING, VERB_CREATE, VERB_DELETE, VERB_DELETE_MANY, VERB_GET_LIST, VERB_GET_MANY, VERB_GET_MANY_REFERENCE, VERB_GET_ONE, VERB_UPDATE, VERB_UPDATE_MANY } from './types';
// cache for all types
var typeMap;
var queryMap;
...
export var buildQuery = function (introspectionResults, factory) { return function (raFetchType, resName, params) {
    ...
    var idField = type.fields.find(function (thisType) { return thisType.name === 'id'; });
    ...
    switch (raFetchType) {
        case VERB_GET_ONE:
            return {
                query: gql(templateObject_1 || (templateObject_1 = __makeTemplateObject(["query ", "($id: ", "!) {\n            ", "(id: $id) {\n            ", "\n        }\n        }"], ["query ", "($id: ", "!) {\n            ", "(id: $id) {\n            ",
                    "\n        }\n        }"])), singleLowerResourceName, idType.name, singleLowerResourceName, createQueryFromType(resourceTypename, typeMap, allowedComplexTypes)),
                variables: {
        ...
        case VERB_GET_MANY:
            return {
                query: createGetManyQuery(type, manyLowerResourceName, resourceTypename, typeMap, queryMap, allowedComplexTypes, idType.name),
                variables: {
                    ids: params.ids
                        .filter(function (v) { return typeof v !== 'undefined'; })
                        .map(function (id) { return mapType(idType, id); })
                },
                parseResponse: function (response) {
        ...
        case VERB_GET_MANY_REFERENCE:
            return getManyReference(params, type, manyLowerResourceName, resourceTypename, typeMap, queryMap, allowedComplexTypes);
        case VERB_GET_LIST: {
            var _b = params, filter = _b.filter, sort = _b.sort;
            var orderBy = sort
                ? [createSortingKey(sort.field, sort.order)]
                : [NATURAL_SORTING];
            var filters = createFilter(filter, type);
            return {
                query: createGetListQuery(type, manyLowerResourceName, resourceTypename, typeMap, queryMap, allowedComplexTypes),
                variables: {
        ...
        case VERB_CREATE: {
            var variables = {
                input: (_a = {},
                    _a[singleLowerResourceName] = mapInputToVariables(params.data, typeMap[resourceTypename + "Input"], type, options.queryValueToInputValueMap),
                    _a)
            };
            return {
                variables: variables,
                query: gql(templateObject_2 || (templateObject_2 = __makeTemplateObject(["mutation create", "($input: Create", "Input!) {\n          create", " (\n          input: $input\n        ) {\n          ", " {\n          ", "\n        }\n        }\n        }"], ["mutation create", "($input: Create", "Input!) {\n          create", " (\n          input: $input\n        ) {\n          ", " {\n          ", "\n        }\n        }\n        }"])), resourceTypename, resourceTypename, resourceTypename, singleLowerResourceName, createQueryFromType(resourceTypename, typeMap, allowedComplexTypes)),
        ...
        case VERB_DELETE_MANY: {
            ...
            var deletions = thisIds.map(function (id) { return ({
            ...
            return {
                variables: deletions.reduce(function (next, input) {
                    var _a;
                    return (__assign((_a = {}, _a["arg" + input.id] = input, _a), next));
                }, {}),
                query: gql(templateObject_4 || (templateObject_4 = __makeTemplateObject(["\n            mutation deleteMany", "(\n            ", "\n            ) {\n            ", "\n            }\n        "], ["\n            mutation deleteMany", "(\n            ",
                    "\n            ) {\n            ",
                    "\n            }\n        "])), resourceTypename, thisIds
                    .map(function (id) { return "$arg" + id + ": Delete" + resourceTypename + "Input!"; })
                    .join(','), params.ids.map(function (id) { return "\n                k" + id + ":delete" + resourceTypename + "(input: $arg" + id + ") {\n                  clientMutationId\n                }\n\n                "; })),
                parseResponse: function (response) { return ({
        ...
        case VERB_UPDATE: {
            var updateVariables = {
                input: {
                    id: mapType(idType, params.id),
                    patch: mapInputToVariables(params.data, typeMap[resourceTypename + "Patch"], type, options.queryValueToInputValueMap)
                }
            };
            return {
                variables: updateVariables,
                query: gql(templateObject_5 || (templateObject_5 = __makeTemplateObject(["\n          mutation update", "($input: Update", "Input!) {\n            update", "(input: $input) {\n            ", " {\n            ", "\n          }\n          }\n          }\n        "], ["\n          mutation update", "($input: Update", "Input!) {\n            update", "(input: $input) {\n            ", " {\n            ",
                    "\n          }\n          }\n          }\n        "])), resourceTypename, resourceTypename, resourceTypename, singleLowerResourceName, createQueryFromType(resourceTypename, typeMap, allowedComplexTypes)),
                parseResponse: function (response) { return ({
        ...
        case VERB_UPDATE_MANY: {
            ...
            var inputs = ids_1.map(function (id) { return ({
                id: mapType(idType, id),
                clientMutationId: id.toString(),
                patch: mapInputToVariables(data_1, typeMap[resourceTypename + "Patch"], type, options.queryValueToInputValueMap)
            }); });
            return {
                ...
                query: gql(templateObject_6 || (templateObject_6 = __makeTemplateObject(["mutation updateMany", "(\n        ", ") {\n          ", "\n        }"], ["mutation updateMany", "(\n        ",
                    ") {\n          ",
                    "\n        }"])), resourceTypename, ids_1
                    .map(function (id) { return "$arg" + id + ": Update" + resourceTypename + "Input!"; })
                    .join(','), inputs.map(function (input) {
                    return "\n             update" + input.id + ":update" + resourceTypename + "(input: $arg" + input.id + ") {\n               clientMutationId\n             }\n            ";
                })),
                parseResponse: function (response) { return ({
                    data: ids_1.map(function (id) {
                        return mapType(idType, response.data["update" + id].clientMutationId);
                    })
                }); }
            };
        }
